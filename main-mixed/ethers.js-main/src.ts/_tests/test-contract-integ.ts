...
describe("Tests contract integration", function() {
    ...
    it("runs contract operations", async function() {
        ...
        const CustomContract = ethers.BaseContract.buildClass<abi>(abi);

        const contract = new CustomContract(address, signer); //ethers.Contract.from<abi>(address, abi, signer);
